...
public final class MonkBurstHealAbility implements HeroAbility {
    public static final String MONK_ROLE_ID = "HyGen_Companion_Monk";
    ...
    public boolean tryActivate(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> playerRef) {
        ...
        if (activeSlot == null || !isMonkSlot(playerRef, activeSlot)) {
            return false;
        }
        ...
        Ref<EntityStore> lowestHealthAlly = null;
        ...
        for (Ref<EntityStore> allyRef : collectAlliedTargets(ownerRef)) {
            ...
            float percent = EntityHealthUtil.readHealthPercent(store, allyRef);
            if (percent < lowestHealthPercent) {
                lowestHealthPercent = percent;
                lowestHealthAlly = allyRef;
            }
        }

        // Everyone is full health, don't waste the ability
        if (lowestHealthAlly == null || lowestHealthPercent >= 1.0f) {
            return false;
        }

        // Heal to max
        EntityHealthUtil.setHealthPercentOfMax(store, lowestHealthAlly, 1.0f);
    ...
    public boolean isAbilityActive(int slot) {
        return false; // Instant ability, no active duration
    }

    public static boolean isMonkSlot(@Nonnull Ref<EntityStore> partyRef, int slot) {
        Integer roleIndex = TrackedSummonStore.getPartyRoleIndex(partyRef, slot);
        if (roleIndex == null) {
            return false;
        }
        String roleName = NPCPlugin.get().getName(roleIndex);
        return MONK_ROLE_ID.equals(roleName);
    }
