...
public class EndGameCommand extends AbstractPlayerCommand {
    public EndGameCommand() {
        super("endgame", "End the active Battleheart run early and clean up the run world");
    }
    ...
    protected void execute(
        ...
        LevelSessionManager.get().finishActiveRun(false).whenComplete((result, throwable) -> {
            if (throwable != null) {
                String reason = throwable.getCause() != null ? throwable.getCause().getMessage() : throwable.getMessage();
                playerRef.sendMessage(Message.raw("Failed to end game: " + reason));
                return;
            }
